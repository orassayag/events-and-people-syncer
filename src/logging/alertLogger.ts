...
export class AlertLogger {
  ...
  private parseAlertEntry(block: string, type: AlertType): Alert | null {
    ...
    for (const line of lines) {
      ...
      if (cleanLine.startsWith('Index:')) {
        index = parseInt(
          cleanLine.replace('Index:', '').replace(',', '').trim(),
          10
        );
        ...
        cleanLine.startsWith('Possible matches:') ||
        cleanLine.startsWith('===Matches:===')
      ) {
        parsingMatches = true;
        ...
        const target =
          parsingMatches && matches.length > 0
        ...
        if (cleanLine.startsWith('👤 Full name:')) {
          const parts = cleanLine
            ...
            .trim()
            .split(' ')
            .filter((p) => !p.toLowerCase().includes('linkedin') && p !== '');
          ...
          cleanLine.startsWith('🔗 LinkedIn URL:') ||
          cleanLine.startsWith('-LinkedIn URL:')
        ) {
          const url = cleanLine
    ...
    if (isNewFormat && alert.type === 'warning') {
      ...
      if (alert.matches && alert.matches.length > 0) {
        ...
        alert.matches.forEach((m, idx) => {
          const scoreText =
            m.score !== undefined ? ` (Score: ${m.score.toFixed(2)})` : '';
          entry.push(
            `${idx + 1}: ${m.firstName} ${m.lastName || ''}${scoreText}`
          );
        });
      }
      entry.push(`=======================`);
  ...
  async writeAlert(
    ...
    const currentFileAlerts = this.allAlerts.filter(
      (_, idx) =>
