...
`;
  ...
  describe('file operations', () => {
    ...
    it('should remove specific alert by index', async () => {
      ...
      const totalAfter =
        allAlertsAfter.warnings.length + allAlertsAfter.errors.length;
  ...
  describe('alert counts and retrieval', () => {
    it('should return correct alert counts', async () => {
      ...
      await logger.writeAlert(
        'warning',
        { firstName: 'W1', lastName: 'User', email: 'w1@example.com' },
        'Test'
      );
      await logger.writeAlert(
        'warning',
        { firstName: 'W2', lastName: 'User', email: 'w2@example.com' },
        'Test'
      );
      await logger.writeAlert(
        'error',
        { firstName: 'E1', lastName: 'User', email: 'e1@example.com' },
        'Test'
      );
      await logger.writeAlert(
        'skipped',
        { firstName: 'S1', lastName: 'User', email: 's1@example.com' },
        'Test'
      );
      const counts = logger.getAlertCounts();
...
`;
      ...
      await logger.writeAlert(
        'warning',
        { firstName: 'New', lastName: 'Alert', email: 'new@example.com' },
        'New'
      );
