...
describe(
  'LinkedIn Sync - Full Dry Mode Integration',
  { timeout: 30000 },
  () => {
    ...
    it('should complete full sync in dry-mode with mocks tracked', async () => {
      ...
      const recentlyModified = (duplicateDetector as any)
        .recentlyModifiedContacts;
    ...
    it('should detect duplicates with mock contacts', async () => {
      (SETTINGS as any).dryMode = true;
      const result1 = await contactSyncer.addContact(
        mockConnections[0],
        'TestLabel'
      );
    ...
    it('should return existing group without creating new one', async () => {
      ...
      const trackWriteSpy = vi.spyOn(apiTracker, 'trackWrite');
      trackWriteSpy.mockClear();
      const group1 = await (contactSyncer as any).ensureGroupExists(
        'ExistingGroup'
      );
      const writeCount1 = trackWriteSpy.mock.calls.length;
      const group2 = await (contactSyncer as any).ensureGroupExists(
        'ExistingGroup'
      );
    ...
    it('should maintain mock contact state across operations', async () => {
      ...
      expect(result1.status).toBe(SyncStatusType.NEW);
      const recentlyModified1 = (duplicateDetector as any)
        .recentlyModifiedContacts;
      expect(recentlyModified1.length).toBe(1);
      const result2 = await contactSyncer.addContact(
        mockConnections[1],
        'TestLabel'
      );
      expect(result2.status).toBe(SyncStatusType.NEW);
      const recentlyModified2 = (duplicateDetector as any)
        .recentlyModifiedContacts;
      expect(recentlyModified2.length).toBe(2);
      const result3 = await contactSyncer.addContact(
        mockConnections[2],
        'TestLabel'
      );
      expect(result3.status).toBe(SyncStatusType.NEW);
      const recentlyModified3 = (duplicateDetector as any)
        .recentlyModifiedContacts;
