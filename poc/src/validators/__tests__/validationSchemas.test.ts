...
describe('ValidationSchemas', () => {
  describe('email', () => {
    it('should validate correct email addresses', () => {
      ...
      expect(
        ValidationSchemas.email.safeParse('name+tag@example.com').success
      ).toBe(true);
    });
    ...
    it('should reject invalid emails', () => {
      ...
      expect(ValidationSchemas.email.safeParse('@example.com').success).toBe(
        false
      );
    });
  ...
  describe('phone', () => {
    it('should validate phone numbers with 1-100 digits', () => {
      ...
      expect(ValidationSchemas.phone.safeParse('123456789012345').success).toBe(
        true
      );
      expect(ValidationSchemas.phone.safeParse('123#456*789').success).toBe(
        true
      );
    ...
    it('should reject phones with too many digits', () => {
      ...
      expect(ValidationSchemas.phone.safeParse(tooManyDigits).success).toBe(
        false
      );
    });
    ...
    it('should reject phones with invalid characters', () => {
      expect(ValidationSchemas.phone.safeParse('123-456-7890abc').success).toBe(
        false
      );
      expect(ValidationSchemas.phone.safeParse('phone number').success).toBe(
        false
      );
    });
  });
  ...
  describe('linkedinUrl', () => {
    ...
    it('should reject non-LinkedIn URLs', () => {
      expect(
        ValidationSchemas.linkedinUrl.safeParse('https://facebook.com/profile')
          .success
      ).toBe(false);
      expect(
        ValidationSchemas.linkedinUrl.safeParse('https://example.com').success
      ).toBe(false);
    });

    it('should reject LinkedIn URLs without valid paths', () => {
      expect(
        ValidationSchemas.linkedinUrl.safeParse('https://linkedin.com').success
      ).toBe(false);
      expect(
        ValidationSchemas.linkedinUrl.safeParse('https://linkedin.com/about')
          .success
      ).toBe(false);
    });
  });
  ...
  describe('fieldLength', () => {
    ...
    it('should reject fields over 1024 characters', () => {
      expect(
        ValidationSchemas.fieldLength.safeParse('a'.repeat(1025)).success
      ).toBe(false);
    });
  });
  ...
  describe('redirectPort', () => {
    ...
    it('should reject ports below 1024', () => {
      ...
      expect(ValidationSchemas.redirectPort.safeParse(1023).success).toBe(
        false
      );
    });

    it('should reject ports above 65535', () => {
      expect(ValidationSchemas.redirectPort.safeParse(65536).success).toBe(
        false
      );
    ...
    it('should reject non-integer ports', () => {
      expect(ValidationSchemas.redirectPort.safeParse(3000.5).success).toBe(
        false
      );
    });
  });
